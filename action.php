...
if(isset($_POST["category"])){
	...
	echo "
		<div class='nav nav-pills nav-stacked'>
			<li class='active' ><a href='#' style='background-color:#5dc460'><h4>Categoria</h4> </a></li>
	";
	if(mysqli_num_rows($run_query) > 0){
		while($row = mysqli_fetch_array($run_query)){
			...
			echo "
					<li><a href='#' class='category' style='color:black' cid='$cid'>$cat_name</a></li>
			";
		}
		echo "</div>";
	}
...
if(isset($_POST["page"])){
	...
	for($i=1;$i<=$pageno;$i++){
		echo "
			<li><a href='#' page='$i' id='page' style='color:white'>$i</a></li>
		";
	}
}
			...
			echo "
				<div class='col-md-4'>
							<div class='panel panel-info'>
								<div class='panel-heading'  style='background-color:#5dc460; border-color:#5dc460;color:white'>$pro_title</div>
								<div class='panel-body'>
									<img src='product_images/$pro_image' align='center' style='width:260px; height:250px;'/>
								</div>
								<div class='panel-heading'style='background-color:#e2e3e7; border-color:#e2e3e7;color:black'>S/.$pro_price.00
									<button pid='$pro_id' style='float:right;background-color:#5dc460; border-color:#5dc460; color:white;' id='product' class='btn btn-danger btn-xs' >Añadir al carrito</button>
								</div>
							</div>
						</div>	
			";
	...
	if(isset($_POST["addToProduct"])){
		...
		if($count > 0){
			...
			echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Lo siento ..! Ir y Registrese Primero,luego podra agregar un producto a su carrito</b>
					</div>
				";
		}
...
if(isset($_POST["get_cart_product"]) || isset($_POST["cart_checkout"])){
	...
	if($count > 0){
		...
		echo '
				...
				<br>
				<input style="float:right;margin-right:30px;margin-bottom:-20;" type="image" name="submit"
